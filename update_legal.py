...
pages = ['privacy-policy.html', 'affiliate-disclosure.html']
...
for f in pages:
    if os.path.exists(f):
        ...
        content_match = re.search(r'<div class="article-content".*?>(.*?)</div>\s*</div>\s*</section>', html, re.DOTALL)
            
        content = content_match.group(1) if content_match else "Content not found"
