...
BE_DIR = f"{WORKSPACE}/content/broke_economist/v3_test"
...
log("1. Generating Script & Imagen Prompts via Gemini...")
...
Format the output EXACTLY as valid JSON with no markdown formatting:
{
  ...
  "prompts": [
    "A photorealistic shot of a sad millennial paying for coffee with a gold bar, cinematic lighting",
    "A photorealistic shot of a giant corporate coffee cup crushing a small wallet, dark studio lighting",
    "A photorealistic shot of a yacht made entirely out of coffee beans, hyperrealistic"
  ]
}
Provide EXACTLY 3 prompts. Make them visually striking for Google Imagen 4."""
...
log("4. Generating Images with Kie.ai (Imagen 4)...")
...
for p in data["prompts"]:
    ...
    if 'data' in r and 'taskId' in r['data']:
        ...
        log(f"Started Imagen task: {r['data']['taskId']}")
    else:
        log(f"Imagen task failed to start: {r}")

log("Polling Imagen tasks...")
images = []
for t in tasks:
    done = False
    for _ in range(30): # Up to 5 mins
        try:
...
img_paths = []
for i, v_url in enumerate(images):
    path = f"{BE_DIR}/img_{i}.jpg"
...
if not img_paths:
    log("No images generated. Exiting.")
    sys.exit(1)

log("5. Stitching Video with Ken Burns effect...")
...
clip_duration = total_duration / len(img_paths)

# Apply Ken Burns to each image
video_clips = []
for i, img in enumerate(img_paths):
    out_clip = f"{BE_DIR}/vid_{i}.mp4"
    # Simple zoom effect
    cmd = [
        "ffmpeg", "-y", "-loop", "1", "-i", img,
        "-vf", f"zoompan=z='min(zoom+0.0015,1.5)':d={int(clip_duration*25)}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)',scale=1080:1920",
        "-c:v", "libx264", "-t", str(clip_duration), "-pix_fmt", "yuv420p",
        out_clip
    ]
...
with open(f"{BE_DIR}/concat.txt", "w") as f:
    for vc in video_clips:
        f.write(f"file '{os.path.basename(vc)}'\n")
...
if res.returncode != 0:
    ...
    log(f"✅ V3 Pipeline Render Complete! Saved to {final_vid}")
