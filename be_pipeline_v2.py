...
BE_DIR = f"{WORKSPACE}/content/broke_economist/v2_test"
...
log("1. Generating Script & Kling Prompts via Gemini...")
genai.configure(api_key=GEMINI_API_KEY)
model = genai.GenerativeModel('gemini-2.5-flash')
prompt = """You are 'The Broke Economist'. Write a 45-second YouTube Short script about 'Shrinkflation'.
...
Format the output EXACTLY as valid JSON with no markdown formatting:
{
  ...
  "prompts": [
    "A photorealistic 3D animated fat cat banker eating a tiny cookie, dark studio lighting",
    "A 3D animated grocery store shelf where the products are shrinking, dramatic lighting",
    "A cynical guy in a suit shrugging while holding an empty wallet"
  ]
}
Provide EXACTLY 3 prompts. Make them visually striking for Kling AI 9:16 aspect ratio."""
...
log("4. Generating Videos with Kling...")
...
for p in data["prompts"]:
    ...
    if 'data' in r and 'taskId' in r['data']:
        ...
        log(f"Started Kling task: {r['data']['taskId']}")
    else:
        log(f"Kling task failed to start: {r}")

log("Polling Kling tasks (this takes 5-10 mins)...")
videos = []
for t in tasks:
    done = False
    for _ in range(120): # Up to 20 mins
        try:
...
with open(f"{BE_DIR}/concat.txt", "w") as f:
    # Loop over the videos a few times so the stream_loop has enough data
    for _ in range(5):
...
if res.returncode != 0:
    ...
    log(f"✅ V2 Pipeline Render Complete! Saved to {final_vid}")
