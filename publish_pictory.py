...
# 1. Read the draft
draft_path = "content/pictory_ai_2026-05-05.md"
with open(draft_path, "r") as f:
    md_content = f.read()

# Parse MD
title_match = re.search(r'\*\*Title:\*\*\s*(.+)$', md_content, re.MULTILINE)
title = title_match.group(1).strip() if title_match else "Pictory AI Review"
...
if body_match:
    body_md = body_match.group(1).strip()
else:
    body_match2 = re.search(r'## Review Body(.*)', md_content, re.DOTALL)
    body_md = body_match2.group(1).strip() if body_match2 else md_content
...
logo_url = "https://logo.clearbit.com/pictory.ai"
    ...
    <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-3xl p-8 sm:p-12 shadow-sm border border-gray-200">
            <div class="flex items-center gap-4 mb-8">
                <img src="{logo_url}?size=128" onerror="this.style.display='none'" class="w-16 h-16 rounded-xl border border-gray-100 shadow-sm" alt="Pictory Logo">
                <div>
                    <h1 class="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">{title}</h1>
                    <p class="text-gray-500 mt-2 text-sm font-medium">Published on May 5, 2026</p>
                </div>
            </div>
...
card_html = f"""
            <a href="{slug}.html" class="bg-card border border-border p-6 rounded-2xl shadow-subtle hover:shadow-hover transition-all group flex flex-col h-full">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <img src="{logo_url}?size=64" onerror="this.style.display='none'" class="w-8 h-8 rounded-md shadow-sm border border-border object-cover" alt="Pictory Logo">
                </div>
...
hub_card_html = f"""
            <a href="/blog/{slug}.html" class="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-md transition-all group flex flex-col h-full">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <img src="{logo_url}?size=64" class="w-10 h-10 rounded-md border border-gray-100" alt="Pictory Logo">
                </div>
...
if '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">' in hub_idx:
    ...
    with open(hub_path, "w") as f:
        f.write(hub_idx)
elif '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="toolsGrid">' in hub_idx:
    hub_idx = hub_idx.replace('<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="toolsGrid">', '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="toolsGrid">\n' + hub_card_html)
    with open(hub_path, "w") as f:
        f.write(hub_idx)
...
with open(csv_file, 'r') as f:
    reader = csv.reader(f)
    for row in reader:
        if len(row) >= 4 and row[1] == 'Pictory AI':
            row[3] = 'Published'
        rows.append(row)
...
print("Pictory processed successfully")