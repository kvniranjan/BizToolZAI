...
if review_match:
    ...
    if review_match:
        ...
        review_body = "Synthesia is a powerhouse in AI video creation."

slug = "synthesia-review-2026"
domain = "synthesia.io"

# 3 & 4. Write new HTML to blog/slug.html
...
<body class="antialiased font-sans">
    ...
    <main class="py-32 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <img src="https://logo.clearbit.com/{domain}?size=128" alt="{tool_name} Logo" class="mb-8 rounded-xl shadow-sm">
...
card_html = f"""            <a href="{slug}.html" class="bg-card border border-border p-6 rounded-2xl shadow-subtle hover:shadow-hover transition-all group flex flex-col h-full">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <img src="https://logo.clearbit.com/{domain}?size=64" onerror="this.style.display='none'" class="w-8 h-8 rounded-md shadow-sm border border-border object-cover" alt="{tool_name} Logo">
                </div>
...
uc_card = f"""            <!-- SYNTHESIA CARD -->
            <a href="/blog/{slug}.html" class="bg-card border border-border p-6 rounded-2xl shadow-subtle hover:shadow-hover transition-all group flex flex-col h-full">
                ...
                <h3 class="text-xl font-bold text-foreground mb-3 group-hover:text-primary transition-colors">{tool_name} Review 2026</h3>
                <p class="text-muted-foreground text-sm mb-6 flex-grow">{meta.split('.')[0]}.</p>
                <div class="flex justify-between items-center text-sm font-semibold text-secondary pt-4 border-t border-border">
...
uc_target = '<div class="grid grid-cols-1 md:grid-cols-2 gap-8">'
